...
A tile bounds transient token-row work. Tiles do not independently invoke MLA
attention: compressed latent state/cache staging is tiled, then attention runs
once with the original full metadata. Token-wise FFN/MoE work may still use an
independent microbatch size.
"""
...
def _parse_size(additional_config: object, key: str, default: int) -> int:
    ...
    try:
        ...
        raise ValueError(f"{key} must be a non-negative integer, got {raw!r}") from exc
...
def parse_pcp_ffn_microbatch_size(additional_config: object, tile_size: int) -> int:
...
def run_tokenwise_microbatches(
    ...
    **kwargs: Any,
    ...
    for token_slice in slices:
        ...
        if output is None:
            output = chunk.new_empty((hidden_states.shape[0], *chunk.shape[1:]))
        output[token_slice].copy_(chunk)
    assert output is not None
    return output


def _validate_config(vllm_config: VllmConfig, tile_size: int, ffn_size: int) -> None:
    if tile_size == 0:
        return
    ...
    if pc.tensor_parallel_size != 1 or pc.data_parallel_size != 1:
        raise NotImplementedError("pcp_tile_size currently requires TP=1 and DP=1.")
    ...
    if pc.use_sequence_parallel_moe:
        raise NotImplementedError("pcp_tile_size does not support sequence-parallel MoE.")
    if sc.max_num_seqs != 1:
        raise NotImplementedError("pcp_tile_size currently requires max_num_seqs=1.")
    if not isinstance(ac, dict) or "pcp_partition_weights" not in ac:
        raise NotImplementedError("pcp_tile_size currently requires weighted PCP.")
    if not mc.use_mla or hasattr(mc.hf_text_config, "index_topk"):
        raise NotImplementedError("pcp_tile_size currently supports dense MLA only.")
    if vllm_config.compilation_config.cudagraph_mode != CUDAGraphMode.NONE:
        raise NotImplementedError("pcp_tile_size currently requires cudagraph_mode=NONE.")
    if vllm_config.cache_config.calculate_kv_scales:
        raise NotImplementedError("pcp_tile_size does not support runtime KV scales.")
    if ffn_size <= 0:
        raise ValueError(f"{_FFN_KEY} must be positive when tiling is enabled")
...
def _supports_tiled_mla(wrapper: Any, num_tokens: int, tile_size: int) -> bool:
    if wrapper.q_lora_rank is None or wrapper.is_sparse or wrapper.indexer is not None:
        return False
    ...
    if metadata is None or int(getattr(metadata, "num_prefills", 0) or 0) <= 0:
        return False
    if wrapper.mla_attn.use_pcp and slot_mapping is not None:
        if slot_mapping.numel() % 2:
            raise RuntimeError("PCP slot mapping does not contain two equal rank slabs.")
        work_rows = slot_mapping.numel() // 2
    else:
        work_rows = num_tokens
    return work_rows > tile_size
...
def _run_tiled_mla(
    ...
    # Phase A is tiled so the fused projection output scales with tile_size.
    for token_slice in tile_slices(num_tokens, tile_size):
        ...
        if wrapper.rotary_emb is not None:
            k_pe_tile, _ = wrapper.rotary_emb(positions[token_slice], k_pe_tile, None)

        if q_c_buffer is None:
            q_c_buffer = q_c_tile.new_empty((num_tokens, *q_c_tile.shape[1:]))
            kv_c_buffer = kv_c_tile.new_empty((num_tokens, *kv_c_tile.shape[1:]))
            k_pe_buffer = k_pe_tile.new_empty((num_tokens, *k_pe_tile.shape[1:]))
        q_c_buffer[token_slice].copy_(q_c_tile)
        kv_c_buffer[token_slice].copy_(kv_c_tile)
        k_pe_buffer[token_slice].copy_(k_pe_tile)

    assert q_c_buffer is not None and kv_c_buffer is not None and k_pe_buffer is not None
    mla_attn = wrapper.mla_attn
    metadata, slot_mapping = _forward_state(mla_attn)

    from vllm.model_executor.layers.attention.pcp import iter_tiled_mla_cache_inputs

    # Cache staging uses tile-sized P2P buffers. Rank1 updates remote and local
    # cache slots separately, avoiding a full remote/local concatenation.
    for cache_kv, cache_kpe, cache_slots in iter_tiled_mla_cache_inputs(
        kv_c_buffer,
        k_pe_buffer,
        slot_mapping,
        metadata.num_decode_tokens if metadata is not None else None,
        mla_attn.use_pcp,
        tile_size,
    ):
    ...
    # Do not tile query attention: one full-metadata invocation avoids repeated
    # prefix/context expansion in the MLA prefill backend.
...
def configure_pcp_tiling(vllm_config: VllmConfig) -> int:
    ...
    ffn_size = parse_pcp_ffn_microbatch_size(vllm_config.additional_config, tile_size)
    ...
    def mla_forward(self, positions, hidden_states, llama_4_scaling=None):
        if not _supports_tiled_mla(self, hidden_states.shape[0], tile_size):
            return original_mla(self, positions, hidden_states, llama_4_scaling)
        return _run_tiled_mla(self, positions, hidden_states, llama_4_scaling, tile_size)

    def ds_mlp_forward(self, hidden_states):
        return run_tokenwise_microbatches(
            lambda x: original_ds_mlp(self, x), hidden_states, ffn_size
        )

    def ds_moe_forward(self, hidden_states, already_sequence_parallel=False):
        if already_sequence_parallel:
            return original_ds_moe(
                self, hidden_states, already_sequence_parallel=already_sequence_parallel
            )
        return run_tokenwise_microbatches(
            lambda x: original_ds_moe(self, x, already_sequence_parallel=False),
            hidden_states,
            ffn_size,
        )
    ...
    def glm_mlp_forward(self, hidden_states):
        return run_tokenwise_microbatches(
            lambda x: original_glm_mlp(self, x), hidden_states, ffn_size
        )

    def glm_moe_forward(self, hidden_states):
        return run_tokenwise_microbatches(
            lambda x: original_glm_moe(self, x), hidden_states, ffn_size
        )
    ...
    _PATCHED = True
    logger.info(
        "Enabled PCP tiling: tile_size=%d, ffn_microbatch_size=%d; MLA attention remains full-metadata",
        tile_size,
        ffn_size,
    )
    return tile_size
